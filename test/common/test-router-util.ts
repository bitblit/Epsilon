...
describe('#routerUtilApplyOpenApiDoc', function() {
  it('should create a router config from a yaml file', async () => {
    ...
    const find: ProxyResult = await webHandler.findHandler(evt, {} as Context, false);
  ...
  it('should find the most specific route and the least specific', async () => {
    ...
    const findFixed: ProxyResult = await webHandler.findHandler(evtFixed, {} as Context, false);
    const findVariable: ProxyResult = await webHandler.findHandler(evtVar, {} as Context, false);
