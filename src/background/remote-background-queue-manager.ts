 ...
 */
export class RemoteBackgroundQueueManager implements BackgroundQueueManager {
  constructor(private awsCfg: BackgroundAwsConfig, private inValidator: BackgroundEntryValidator) {}

  public validator(): BackgroundEntryValidator {
    return this.inValidator;
  }

  public async addEntryToQueue(entry: BackgroundEntry, fireStartMessage: boolean = true): Promise<string> {
  ...
  public async fireStartProcessingRequest(): Promise<string> {
    return this.writeMessageToSnsTopic(EpsilonConstants.BACKGROUND_SNS_START_MARKER);
  }
