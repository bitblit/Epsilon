...
export class OldWebHandler {
  ...
  public async openApiLambdaHandler(event: APIGatewayEvent, context: Context): Promise<ProxyResult> {
    ...
    if (proxyResult.body.length > OldWebHandler.MAXIMUM_LAMBDA_BODY_SIZE_BYTES) {
      const delta: number = proxyResult.body.length - OldWebHandler.MAXIMUM_LAMBDA_BODY_SIZE_BYTES;
